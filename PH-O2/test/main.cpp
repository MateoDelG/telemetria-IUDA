...
void taskCore1(void *pvParameters) {
  for (;;) {
    ...
    // Serial2.println("hola master");
      ...
      //     remoteManager.log("RX: "); remoteManager.log(s);
      //     got = true; break;
      //   }
      // }
      // if (!got) remoteManager.log("SIN ECO (pines no funcionan para UART2 aquí)");
      // delay(500);
...
static void MenuDemoTick() {
  ...
  switch (view) {
      ...
      switch (readLatched()) {
        ...
        case Btn::OK:
          if      (rootCursor==0) { view = View::MANUAL_MENU; renderManualMenu(); }
            else if (rootCursor==1) {          // <-- Automatico
              autoFirst = true;
              view = View::AUTO;
              lcd.clear();
            } else {
              view = View::CONFIG; renderConfig();
            }
        default: break;
      }
    } break;
    ...
    // --------- Automático ----------
    case View::AUTO: {
      // Limpia la pantalla solo la primera vez que entras
      if (autoFirst) { lcd.clear(); autoFirst = false; }

      // Ejecuta un “tick” del modo automático.
      // Devuelve true si terminó (completado o cancelado).
      if (AutoModeTick()) {
        view = View::ROOT;
        lcd.clear();
        renderRoot();
        autoFirst = true;       // siguiente vez, limpiar otra vez
      }

      // Importante: NO llames readLatched() ni resetees botones aquí.
      // AutoModeTick ya maneja ESC para cancelar y pinta su propia UI.
    } break;
